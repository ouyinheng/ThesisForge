...
import { ref, computed, watch, onMounted, onUnmounted, provide, markRaw, defineAsyncComponent } from 'vue'
...
watch(
  ...
  (path) => {
    ...
    if (currentLayout !== 'empty' && currentLayout !== 'simple' &&
        ...
        <component :is="Layout">
          <transition name="fade-slide" mode="out-in" appear>
            <keep-alive :include="tabs.cachedNames">
              <component :is="Component" :key="curRoute.fullPath + (reloadKeyMap[curRoute.fullPath] || 0)" />
            </keep-alive>
          </transition>
        </component>
      </router-view>
